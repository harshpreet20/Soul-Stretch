...
export default function WhatsAppButton() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 0, text: "Hey! How can we help you today?", sender: 'bot' },
  ])
  ...
  const handleSendMessage = async () => {
    ...
    if (step === 'chat' && !name) {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, text: "Thanks! Could you share your name and email so we can get back to you?", sender: 'bot' },
      ])
      return
    }
    ...
    setMessages((prev) => [
      ...prev,
      {
        id: Date.now() + 1,
        text: success
          ? "Got it! We'll get back to you soon. You can also reach us on WhatsApp for a faster response."
          : "Sorry, something went wrong. Please try WhatsApp instead.",
        sender: 'bot',
      },
    ])
    setIsSending(false)
  ...
  const handleDetailsSubmit = async () => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now() + 1, text: `Thanks ${name}! What can we help you with?`, sender: 'bot' },
    ])
  }
          ...
          >
            ...
            <div className="bg-soul-orange px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-soul-black/20 rounded-full flex items-center justify-center">
                  <span className="text-sm font-bold">SS</span>
                </div>
                <div>
                  <p className="text-sm font-bold text-soul-black">Soul Stretch</p>
                  <p className="text-xs text-soul-black/60">Usually replies within hours</p>
                </div>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                className="text-soul-black/60 hover:text-soul-black transition-colors"
              >
            ...
            {/* Input Area */}
            <div className="border-t border-soul-orange/10 p-3">
              {step === 'details' ? (
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-soul-black border border-soul-orange/20 rounded-lg text-soul-white text-sm placeholder-soul-gray focus:border-soul-orange focus:outline-none"
                  />
                  <input
                    type="email"
                    value={email}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 bg-soul-black border border-soul-orange/20 rounded-lg text-soul-white text-sm placeholder-soul-gray focus:border-soul-orange focus:outline-none"
                  />
                  <button
              ...
              <a
                ...
                className="flex items-center justify-center gap-1.5 mt-2 text-xs text-green-400 hover:text-green-300 transition-colors"
                onClick={() => {
                  sendToN8N({
      ...
      >
        ...
        >
          {isOpen ? (
            ...
            <svg className="w-7 h-7 text-soul-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
